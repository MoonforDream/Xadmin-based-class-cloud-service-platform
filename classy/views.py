...
def login_view_app(request):
    if request.method=='GET':
        ...
        if msg1:
            ...
            message1=''
        return render(request,'login_app.html',locals())
    if request.method=='POST':
        username=request.POST.get('username')
        password=request.POST.get('password')
        user=authenticate(request,username=username,password=password,is_active=1)
        if user:
            login(request,user)
            return redirect('http://101.33.251.175:30032/class/welcome/')
        else:
            error='该用户不存在或者帐号密码错误'
            return render(request,'login_app.html',{'error':error})
...
def task_list(request):
    ...
    if request.method=='POST':
        ...
        if obj.task_type=='签到':
            ...
            distance1=distance(checkin_coordinate,user_coordinate).meters
            print(distance1)
            print(radius)
            if distance1 <= radius:
        ...
        for i in range(num_rows):
            ...
            try:
                ...
                user = UserInfo.objects.create_user(username=username,
                                                    password=password,
                                                    is_active=1, is_superuser=0, class_num=class_id,
                                                    stu_name=df.values[i][2], stu_id=df.values[i][1],
                                                    stunum=df.values[i][0], stu_class=stu_class, is_staff=0)
                user.save()
            except:
                pinyin = lazy_pinyin(df.values[i][1])
                pinyin_abbr = ''.join([s[0] for s in pinyin])
                username = str(uidb64) + str(df.values[i][2]) + pinyin_abbr
                num = str(df.values[i][2])[-4:]  # 学号后四位
                password = 'ZKSD' + pinyin_abbr + num
                obj1 = UserInfo.objects.filter(stu_name=df.values[i][1])
                if obj1:
                    continue
                user = UserInfo.objects.create_user(username=username,
                                                    password=password,
                                                    is_active=1, is_superuser=0, class_num=class_id,
                                                    stu_name=df.values[i][1], stu_id=df.values[i][2],
                                                    stunum=df.values[i][0], stu_class=stu_class, is_staff=0)
                user.save()
...
def register(request):
    if request.method=='POST':
        ...
        if not obj and not obj1:
            user=UserInfo.objects.create_user(username=username,password=password,email=email,is_active=0,is_superuser=0,is_staff=1,stu_work='班长')
            obj1=UserInfo.objects.get(username=username)
            uid=obj1.id
            result = send_sms.delay(email, uid,1)
...
def download_file(request):
    ...
    print(filepath)
    response=sendfile(request,filepath)
    return response
...
def only(request):
    ...
    if request.method=='POST':
        ...
        if 'password' in request.POST:
            ...
            if pan:
                send_pass(user.email,user.id,flag=1)
...
def add(request):
    ...
    if request.method=='POST':
        ...
        password = 'ZKSD' + pinyin_abbr + num
        obj = UserInfo(stu_name=stu_name, username=username, password=make_password(password,None,'pbkdf2_sha256'), stunum=stunum, stu_id=stu_id,
...
def modify_role(request):
    ...
    if request.method == 'GET':
        if user.stu_work in ['班长', '副班长', '学习委员', '学委']:
            message2=request.session.get('message2')
            if message2:
                del request.session['message2']
        ...
        if stunum != '':
            obj.stunum=stunum
        if stu_work != '':
            obj.stu_work=stu_work
        obj.save()
        request.session['message2']='修改成功'
        return redirect('/class/modify_role?name=%s' % username)


@login_required(login_url='/class/login_app/')
    ...
    if request.method == 'POST':
        ...
        if 'start' in request.POST:
            ...
            if start and not end and not taskname:
                ...
                lst = task.objects.filter(task_name__contains=taskname, task_start__contains=start,
                                          task_end__contains=end, clss_id=user.class_num)
            elif start and end and not taskname:
                lst = task.objects.filter(task_start__contains=start, task_end__contains=end, clss_id=user.class_num)
            elif start and not end and taskname:
                lst = task.objects.filter(task_start__contains=start, task_name__contains=taskname,
                                          clss_id=user.class_num)
            elif not start and end and taskname:
                lst = task.objects.filter(task_name__contains=taskname, task_end__contains=end, clss_id=user.class_num)
            if not lst:
                m1 = '未找到该任务'
...
def only_app(request):
    ...
    if request.method == 'POST':
        ...
        if 'password' in request.POST:
            ...
            pan = authenticate(username=login_user, is_active=1, password=oldpass)
            if pan:
                send_pass(user.email, user.id,0)
                request.session['pass'] = password
                message = '密码修改链接已通过邮箱发送成功，请尽快激活!'
            else:
                message1 = '旧密码错误'
...
def logout_view_app(request):
    logout(request)
    return redirect('/class/login_app/')
...
def register_app(request):
    if request.method=='POST':
        ...
        if not obj and not obj1:
            ...
            obj1=UserInfo.objects.get(username=username)
            uid=obj1.id
            result = send_sms.delay(email, uid,0)
            message1='成功注册，邮箱激活后即可使用'
        return render(request,'register_app.html',locals())
    else:
        return render(request,'register_app.html')



def password_app(request):
    if request.method=='GET':
        return render(request,'password_app.html')
    if request.method=='POST':
        username=request.POST.get('username')
        email=request.POST.get('email')
        password=request.POST.get('password')
        ...
        if email==user.email and not user.check_password(password):
            ...
            message='邮箱与帐号邮箱不符！'
        return render(request,'password_app.html',locals())


def to_excel(request):
    user=request.user
    data=UserInfo.objects.filter(is_superuser=0,is_active=1,class_num=user.class_num)
    workbook=openpyxl.Workbook()
    worksheet=workbook.active
    headers=['姓名','帐号']
    worksheet.append(headers)
    for item in data:
        row=[item.stu_name,item.username]
        worksheet.append(row)
    filename = 'account_info.xlsx'
...
def download_app(request):
    path="/home/dreamcreator/class/media/apk/class-clouds-v1-0-3.apk"
    return render(request,"code.html",locals())

def share_app(request):
    return render(request, "code_app.html", locals())

#404
def page404(request,exception,template_name='404.html'):
    return render(request,template_name)

def page403(request,exception,template_name='403.html'):
    return render(request,template_name)

def page400(request,exception,template_name='400.html'):
    return render(request,template_name)
...
def activate_app(request):
    ...
    if is_token:
        if user is not None and token==is_token:
            ...
            request.session['message']='链接激活无效'
            return redirect('/class/register_app/')
    else:
        request.session['message'] ='链接激活无效或已过期'
        return redirect('/class/register_app/')
